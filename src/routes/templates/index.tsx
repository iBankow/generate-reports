      ...
      <div className="max-w-6xl mx-auto">
        ...
        {isLoading ? (
          ...
          <Card className="p-12 text-center route-card">
            <p className="route-text-muted mb-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {templates.map((template) => (
              <Card
                key={template.id}
                className="p-6 flex flex-col hover:shadow-lg transition route-card"
              >
                <h3 className="text-lg font-bold mb-2">{template.title}</h3>
                <p className="text-sm route-text-muted mb-4 flex-1">
                  {template.description}
                </p>
                <div className="text-xs route-text-muted mb-4">
                  {template.fields.length} campo
                  {template.fields.length !== 1 ? 's' : ''}
                </div>
                <div className="flex gap-2">
                  <Button
                    onClick={() => handleUseTemplate(template.id)}
                    className="flex-1 route-button-primary"
                    size="sm"
                  >
                    Usar
                  </Button>
